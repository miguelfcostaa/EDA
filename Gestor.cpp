...
void gravarsuper(string gravac, sector* sectores, int Nsector, prod* produt) {
	...
	for (int i = 0; i < Nsector; i++) {
		...
		for (int j = 0; j < sectores[i].Nproduto; j++) {
			res = res + "\n" + sectores[i].prods[j].produto + "  " + to_string(sectores[i].prods[j].preco);
		}
		res = res + "\n" + "---------------------------------------------------------------------------------------------------------------" + "\n";
	}
	ficheiro.open(gravac, ifstream::out);
...
sector* carregaSuper(string estadoGravado, int Nsector) {
	sector* estado = new sector[Nsector];
	fstream ficheiro(estadoGravado, ios::in);
	string line;
	getline(ficheiro, line);
	char letra;
	string produto; string responsavel; string area;
	int produtos; int preco{}; int capacidade; int i = 0; int j = 0; char c;
...
void mostraregisto(sector* sectores, int Nsector) {
	...
	for (int i = 0; i < Nsector; i++) {
		if (nomeaux == sectores[i].nome) {
			...
			for (int j = 0; j < sectores[j].Nproduto; j++) {
				if (sectores[i].prods[j].vendido == true) { 
					cout << sectores[i].prods[j].produto << " por: " << sectores[i].prods[j].preco << " Euros." << endl;
				}
			}
		}
	}
